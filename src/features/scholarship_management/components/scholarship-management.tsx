...
import { ScholarshipForm, type ScholarshipFormValues } from "./scholarship-form"
import { ScholarshipTable } from "./scholarship-table"
import type { IScholarship } from "@/types/scholarship"
import { Plus } from "lucide-react"
import { useGetMyScholarships, useCreateScholarship, useUpdateScholarship, useDeleteScholarship } from "../hooks/use-scholarship-management"

export function ScholarshipManagement() {
  const { data: scholarships = [], isLoading: isLoadingScholarships } = useGetMyScholarships()
  ...
  const [selectedScholarship, setSelectedScholarship] = useState<IScholarship | undefined>()
    ...
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="mt-1 text-muted-foreground">Manage and track your scholarship applications</p>
        </div>
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="min-w-5xl max-w-7xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>{selectedScholarship ? "Edit Scholarship" : "Create New Scholarship"}</DialogTitle>
            <DialogDescription>
              {selectedScholarship
